...
61. **Fase de Finalização (Versão 1.0.5):**
    - **Ação:** Correção do canal de publicação (Force auto-publish).
    - **Detalhes:**
        - Implementação de feedback visual de download (0-100% na barra de status).
        - Ajuste no escalonamento da janela principal para evitar atraso no IPC.
        - Padronização do sistema de gestão global de produtos.
        - A String da versão na Interface (Barra de Status) foi convertida rigorosamente para coincidir com a label oficial de compilação semântica (v1.0.5) refletindo o estado maduro de Produção/Distribuição.
